...
class ValueHolderPort<T>(val portId: String, private val nodeExecutor: AbstractNodeExecutor) {
...
fun createValueHolder(port: InitialPortModel, nodeExecutor: AbstractNodeExecutor) = when (port) {
  ...
  is DataPortModel -> ValueHolderPort<String>(port.getID(), nodeExecutor)
